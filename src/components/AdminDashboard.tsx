...
import { Service, Course, BlogPost, StudentResult, HomepageConfig, ContactConfig, MentorConfig, EnrollmentApplication } from '../types';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ isOpen, onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<'services' | 'courses' | 'blogs' | 'verifications' | 'homepage' | 'contact' | 'mentor' | 'admissions' | 'settings' | 'siteSettings'>('services');
          ...
          {!token ? (
              ...
              <form onSubmit={handleLogin} className="space-y-4 bg-slate-50 p-6 rounded-2xl border border-slate-200">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="admin@futuregates.edu.pk"
                ...
                <p className="text-[11px] text-center text-slate-500 pt-2">
                  Default credentials: <strong>admin@futuregates.edu.pk</strong> / <strong>admin123</strong>
                </p>
              </form>
            </div>
          ) : (
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap border-b border-slate-200 gap-1 sm:gap-2">
                ...
                >
                  <Users className="w-3.5 h-3.5" /> Leads ({data.admissions?.length || 0})
                </button>
                ...
                <form onSubmit={handleSaveHomepageForm} className="space-y-4 bg-slate-50 p-5 rounded-2xl border border-slate-200 text-xs">
                  ...
                  <div className="p-4 bg-white rounded-xl border border-slate-200 space-y-3">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      <div>
                        <label className="block text-[10px] text-slate-500 font-bold">Students Certified</label>
                        <input
                          type="text"
              ...
              {activeTab === 'admissions' && (
                <div className="space-y-4">
                  <h4 className="font-bold text-sm text-slate-900">Online Enrolment Applications & Student Inquiries</h4>
                  {(!data.admissions || data.admissions.length === 0) ? (
                    <div className="text-center py-10 bg-slate-50 rounded-2xl text-slate-500 text-xs">
                      No online course applications received yet.
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {data.admissions.map((adm) => (
                        <div key={adm.id} className="p-4 bg-slate-50 rounded-xl border border-slate-200 text-xs space-y-1.5">
                          <div className="flex justify-between items-center">
                            <span className="font-bold text-slate-900 text-sm">{adm.fullName}</span>
                            <div className="flex items-center gap-2">
                              <span className="text-[10px] text-slate-400 font-mono">{new Date(adm.submittedAt).toLocaleDateString()}</span>
                              <button
                                onClick={async () => {
                                  if (confirm('Delete this application lead?')) {
                          ...
                          <p className="text-slate-700"><strong>Applied Course:</strong> {adm.courseTitle}</p>
                          <p className="text-slate-600 font-mono"><strong>Phone:</strong> {adm.phone} | <strong>WhatsApp:</strong> {adm.whatsapp}</p>
                          {adm.address && <p className="text-slate-500"><strong>Address:</strong> {adm.address}</p>}
                        </div>
                      ))}
                    </div>
                  )}
